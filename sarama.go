...
func WrapPartitionConsumer(pc sarama.PartitionConsumer, opts ...Option) sarama.PartitionConsumer {
	...
	go func() {
		...
		for msg := range msgs {
			...
			next.SetTag("partition", strconv.FormatInt(int64(msg.Partition),10))
			next.SetTag("offset", strconv.FormatInt(int64(msg.Offset),10))
			next.SetTag("messaging.destination", msg.Topic)
			next.SetTag("span.kind","CONSUMER")
...
func startProducerSpan(cfg *config, version sarama.KafkaVersion, msg *sarama.ProducerMessage) ddtrace.Span {
	...
	span.SetTag("messaging.destination",msg.Topic)
	span.SetTag("message_bus.destination",msg.Topic)
	span.SetTag("span.kind","PRODUCER")
	span.SetTag("region","east")
	span.SetTag("country","US")
	span.SetTag("environment","kafka-testing")
...
func finishProducerSpan(span ddtrace.Span, partition int32, offset int64, err error) {
	span.SetTag("partition", strconv.FormatInt(int64(partition),10))
	span.SetTag("offset", strconv.FormatInt(int64(offset),10))
